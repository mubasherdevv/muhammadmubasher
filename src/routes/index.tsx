...
function Index() {
  ...
  return (
    <div className="min-h-dvh bg-background text-foreground font-sans selection:bg-accent selection:text-accent-foreground">
      ...
      {/* CASE STUDIES */}
      <section id="case-studies" className="px-6 py-32 bg-foreground text-background">
        <div className="mx-auto max-w-[1400px]">
          <div className="grid md:grid-cols-12 gap-12 mb-20">
            <div className="md:col-span-5">
              <div className="text-xs uppercase tracking-[0.3em] text-background/60 mb-4">— Case Studies</div>
              <RevealHeading lines={["Builds, not", "mockups."]} className="text-5xl md:text-7xl" />
            </div>
            <div className="md:col-span-7 flex items-end">
              <Reveal delay={0.15}>
                <p className="text-xl md:text-2xl font-light leading-snug text-background/70">
                  Two shipped products — one web, one mobile — measured on speed, stability,
                  and how fast the team could ship after handover.
                </p>
              </Reveal>
            </div>
          </div>
          ...
          <div className="space-y-6">
            {caseStudies.map((cs, i) => (
              <motion.article
                ...
                className="group relative rounded-3xl border border-background/15 bg-background/5 overflow-hidden"
              >
                <div className="grid lg:grid-cols-12 gap-0">
                  <div
                    className="lg:col-span-4 relative min-h-[240px] lg:min-h-[420px] p-8 flex flex-col justify-between text-white"
                  ...
                  >
                    ...
                    <div className="relative">
                      <div className="text-[10px] uppercase tracking-[0.3em] text-white/80">
                        Case 0{i + 1} · {cs.duration}
                      </div>
                      <div className="text-xs uppercase tracking-widest text-white/70 mt-2">
                        {cs.industry}
                      </div>
                    </div>
                    <h3 className="relative font-display uppercase text-3xl md:text-4xl leading-none">
                      {cs.client}
                    </h3>
                  </div>
                  ...
                  <div className="lg:col-span-8 p-8 md:p-10 flex flex-col gap-8">
                    <div className="grid sm:grid-cols-2 gap-8">
                      <div>
                        <div className="text-[10px] uppercase tracking-[0.3em] text-background/60 mb-3">
                          Challenge
                        </div>
                        <p className="text-sm md:text-base leading-relaxed text-background/85">{cs.challenge}</p>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-[0.3em] text-background/60 mb-3">
                          Approach
                        </div>
                        <p className="text-sm md:text-base leading-relaxed text-background/85">{cs.approach}</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-3 gap-4 pt-6 border-t border-background/15">
                      {cs.metrics.map((m) => (
                        <div key={m.v}>
                          <div className="font-display text-3xl md:text-4xl leading-none">{m.k}</div>
                          <div className="text-[10px] uppercase tracking-widest text-background/60 mt-2">
                            {m.v}
                          </div>
                        </div>
                      ))}
                    ...
                    <blockquote className="text-base md:text-lg italic text-background/70 border-l-2 border-accent pl-4">
                      "{cs.quote}"
                    </blockquote>
                  </div>
                </div>
